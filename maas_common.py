...
try:
    ...
    def get_glance_client(destination, *args, **kwargs):
        status_err('Cannot import glanceclient')
else:
    def get_glance_client(destination, token=None, endpoint=None, previous_tries=0):
        if previous_tries > 3:
            return None
        ...
        try:
            ...
            auth_ref = force_reauth(destination)
            token = auth_ref['token']['id']

            glance = get_glance_client(destination, token, endpoint, previous_tries + 1)
...
try:
    ...
    def get_nova_client(destination, *args, **kwargs):
        status_err('Cannot import novaclient')
else:
    def get_nova_client(destination, auth_token=None, bypass_url=None, previous_tries=0):
        if previous_tries > 3:
            return None
        ...
        # Attribute error. This is a bug, to be filed...
            ...
            nova = get_nova_client(destination, auth_token, bypass_url, previous_tries + 1)
...
try:
    ...
    def keystone_auth(destination, *args, **kwargs):
        status_err('Cannot import keystoneclient')

    def get_keystone_client(destination, *args, **kwargs):
        status_err('Cannot import keystoneclient')
else:
    def keystone_auth(destination, auth_details):
        try:
        ...
        try:
            if destination == 'to':
                tk_file = TOKEN_FILE_TO
            else:
                tk_file = TOKEN_FILE_FROM

            with open(tk_file, 'w') as token_file:
    ...
    def get_keystone_client(destination, auth_ref=None, endpoint=None, previous_tries=0):
        if previous_tries > 3:
            return None
        ...
        try:
            ...
            auth_ref = force_reauth(destination)
            keystone = get_keystone_client(destination, auth_ref,
                                           endpoint,
                                           previous_tries + 1)
...
try:
    ...
    def get_neutron_client(destination, *args, **kwargs):
        status_err('Cannot import neutronclient')
else:
    def get_neutron_client(destination, token=None, endpoint_url=None, previous_tries=0):
        if previous_tries > 3:
            return None
        ...
        # local token) we'll just catch the exception it throws and move on
            ...
            neutron = get_neutron_client(destination, token, endpoint_url,
                                         previous_tries + 1)
...
try:
    ...
    def get_heat_client(destination, *args, **kwargs):
        status_err('Cannot import heatclient')
else:
    def get_heat_client(destination, token=None, endpoint=None, previous_tries=0):
        if previous_tries > 3:
            return None
        ...
        try:
            ...
            auth_ref = force_reauth(destination)
            token = auth_ref['token']['id']
            heat = get_heat_client(destination, token, endpoint, previous_tries + 1)
...
def get_auth_ref(destination):
    auth_details = get_auth_details(destination)
    auth_ref = get_auth_from_file(destination)
    if auth_ref is None:
        auth_ref = keystone_auth(destination, auth_details)
...
def get_auth_from_file(destination):
    try:
        if destination == 'to':
            tk_file = TOKEN_FILE_TO
        else:
        ...
        with open(tk_file) as token_file:
...
def get_auth_details(destination):
...
def force_reauth(destination):
    auth_details = get_auth_details(destination)
    return keystone_auth(destination, auth_details)
...
logging.basicConfig(filename='maas_plugins.log',
